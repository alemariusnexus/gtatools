...
crc32_t Crc32(const char value[]) {
    ...
    for (size_t i = 0 ; i < strlen(value) ; i++) {
        char chr = value[i];
        checksum = __crc_table[((checksum ^ chr)  &  0xFF)]
...
void RwGetSectionName(uint32_t id, char* dest) {
	...
	case RW_SECTION_ATOMIC:
		strcpy(dest, "RW_SECTION_ATOMIC");
		break;
	case RW_SECTION_CLUMP:
		strcpy(dest, "RW_SECTION_CLUMP");
		break;
	case RW_SECTION_EXTENSION:
		strcpy(dest, "RW_SECTION_EXTENSION");
		break;
	case RW_SECTION_FRAME:
		strcpy(dest, "RW_SECTION_FRAME");
		break;
	case RW_SECTION_FRAMELIST:
		strcpy(dest, "RW_SECTION_FRAMELIST");
		break;
	case RW_SECTION_GEOMETRY:
		strcpy(dest, "RW_SECTION_GEOMETRY");
		break;
	case RW_SECTION_GEOMETRYLIST:
		strcpy(dest, "RW_SECTION_GEOMETRYLIST");
		break;
	case RW_SECTION_MATERIAL:
		strcpy(dest, "RW_SECTION_MATERIAL");
		break;
	case RW_SECTION_MATERIALLIST:
		strcpy(dest, "RW_SECTION_MATERIALLIST");
		break;
	case RW_SECTION_MATERIALSPLIT:
		strcpy(dest, "RW_SECTION_MATERIALSPLIT");
		break;
	case RW_SECTION_STRING:
		strcpy(dest, "RW_SECTION_STRING");
		break;
	case RW_SECTION_STRUCT:
		strcpy(dest, "RW_SECTION_STRUCT");
		break;
